...
const SearchResults: React.FC<SearchResultsProps> = ({ searchResults, loading, error }) => {
    ...
    return (
      <div className="space-y-4">
        {Array.from({ length: 5 }).map((_, idx) => (
          <div key={idx} className="white-card p-6 animate-pulse">
  ...
  const ProfileCard: React.FC<{ profile: SearchProfile }> = ({ profile }) => {
    ...
    return (
      <div className="white-card p-6 hover:shadow-md transition-shadow cursor-pointer">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between">
              ...
              <button className="ml-4 px-4 py-2 bg-[#7C0B2B] text-white text-sm rounded-md hover:bg-[#7C0B2B]/90 transition-colors">
                Connect
              </button>
            </div>
          </div>
