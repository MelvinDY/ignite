...
export function HandleSetupPage() {
  ...
  const validateHandle = useCallback((handle: string): string | null => {
    ...
    if (handle.length > 30) return 'Handle must be no more than 30 characters long';
  ...
  useEffect(() => {
    const checkAvailability = async () => {
      ...
      try {
        const available = await profileApi.checkHandleAvailability(debouncedHandle);
    ...
    try {
      ...
      if (err instanceof ProfileApiError) {
        if (err.code === 'HANDLE_TAKEN') {
          setError('This handle is no longer available. Please choose another one.');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#3E000C] to-[#8B1538] flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white/10 backdrop-blur-md rounded-lg p-6 text-white">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-white/80 text-sm leading-relaxed">
            Choose a unique handle to create your profile. This will be your public identifier in the PPIA UNSW community.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white/5 rounded-md p-4">
            <h3 className="text-sm font-medium text-white mb-2">Handle Guidelines:</h3>
            <ul className="text-xs text-white/70 space-y-1">
              <li>• 3-30 characters long</li>
              <li>• Lowercase letters, numbers, dots, underscores, hyphens only</li>
